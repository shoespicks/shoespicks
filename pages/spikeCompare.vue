    ...
    <div class="compareBlock">
      <div class="compareSelect1">
        <b-form-select v-model="spike1" class="mb-3" @change="submit">
          <b-form-select-option-group :label="category">
            <b-form-select-option v-for="selectSpike in options" v-bind:value="selectSpike.value" :key="selectSpike.value" :selected="spike1">{{selectSpike.text}}
            </b-form-select-option>
          </b-form-select-option-group>
        </b-form-select>
        
        <b-img :src= spikeImgUrl1 alt="スパイク比較用画像1"></b-img>
        <b-table striped hover caption-top label="Table Options" :items= table1 
      ...
      <div class="compareSelect2">
        <b-form-select v-model="spike2" class="mb-3" @change="submit">
          ...
          <b-form-select-option-group :label="category">
            <b-form-select-option v-for="selectSpike in options" v-bind:value="selectSpike.value" :key="selectSpike.value" :selected="spike2">{{selectSpike.text}}
            </b-form-select-option>
          </b-form-select-option-group>
        </b-form-select>
        ...
        <b-table striped hover caption-top label="Table Options" :items= table2 
        :fields="[{key: 'key',},{key: 'result',},]">
        </b-table>
...
async asyncData({ payload, query }) {
  ...
  created: function() {
    ...
    this.table1 = [
      ...
      {key: "サイズ", result: this.spike.fields.spikeMinSize + "〜" + this.spike.fields.spikeMaxSize},
    ...
    if(this.spike2!=null){
      var searchInput2: { [key: string]: string } = {
        ...
        this.table2 = [
          ...
          {key: "サイズ", result: e.items[0].fields.spikeMinSize + "〜" + e.items[0].fields.spikeMaxSize},
